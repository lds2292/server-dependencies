...
export async function sendInvitation(projectId: string, inviterId: string, identifier: string, role: ProjectMemberRole) {
  ...
  if (!target) throw Object.assign(new Error('해당 사용자를 찾을 수 없습니다.'), { code: 'NOT_FOUND' })
  ...
  if (existing) {
    if (existing.status === 'PENDING') throw Object.assign(new Error('이미 초대가 전송되었습니다.'), { code: 'ALREADY_INVITED' })
...
export async function respondInvitation(invitationId: string, userId: string, action: 'accept' | 'reject') {
  ...
  if (!invitation) throw Object.assign(new Error('초대를 찾을 수 없습니다.'), { code: 'NOT_FOUND' })
  if (invitation.inviteeId !== userId) throw Object.assign(new Error('접근 권한이 없습니다.'), { code: 'ACCESS_DENIED' })
  if (invitation.status !== 'PENDING') throw Object.assign(new Error('이미 처리된 초대입니다.'), { code: 'ALREADY_PROCESSED' })
  ...
  if (action === 'accept') {
    ...
    if (alreadyMember) {
      ...
      throw Object.assign(new Error('이미 멤버입니다.'), { code: 'ALREADY_MEMBER' })
    }
...
export async function cancelInvitation(invitationId: string, requesterId: string, projectId: string) {
  ...
  const invitation = await prisma.projectInvitation.findUnique({ where: { id: invitationId } })
  if (!invitation || invitation.projectId !== projectId) throw Object.assign(new Error('초대를 찾을 수 없습니다.'), { code: 'NOT_FOUND' })
