...
export async function login(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (e.code === 'INVALID_CREDENTIALS') {
      logger.warn('AUTH login failed', { email: req.body.email, reason: 'INVALID_CREDENTIALS' })
      res.status(401).json({ error: e.message, code: e.code })
    } else {
      logger.error('AUTH login error', { email: req.body.email, error: (err as Error).message })
      res.status(500).json({ error: '서버 오류가 발생했습니다.' })
    }
  }
}
...
export async function logout(req: Request, res: Response): Promise<void> {
  try {
    ...
    logger.info('AUTH logout', { hasToken: !!refreshToken })
    res.status(204).send()
  } catch (err) {
